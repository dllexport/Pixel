...
void VulkanRenderPassExecutor::updateDrawStates()
{
    ...
    auto result = vkAllocateCommandBuffers(context->GetVkDevice(), &commandBufferAllocateInfo, &updateCommandBuffer);

    VkCommandBufferBeginInfo cmdBufferBeginInfo = {};
    cmdBufferBeginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
    vkBeginCommandBuffer(updateCommandBuffer, &cmdBufferBeginInfo);

    bool updateAny = false;
    // update drawStates
    for (auto &[_, drawStates] : resourceBindingStates)
    {
        for (auto &drawState : drawStates)
        {
            auto vulkanDrawState = static_cast<VulkanResourceBindingState *>(drawState.get());
            auto &updateRequests = vulkanDrawState->GetUpdateRequests();
            for (int i = 0; i < updateRequests.size(); i++)
            {
                auto &updateRequest = updateRequests[i];
                VkImageSubresourceRange subresourceRange = {};
                ...
                setImageLayout(updateCommandBuffer,
                               updateRequest.texutre->GetImage(),
                               updateRequest.oldLayout,
                               updateRequest.newLayout,
                               subresourceRange,
                               updateRequest.srcStageMask,
                               updateRequest.dstStageMask);
    ...
    if (!updateAny)
    {
        vkResetCommandBuffer(updateCommandBuffer, VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT);
        return;
    }
    ...
    // Submit to the queue
    result = vkQueueSubmit(context->GetQueue(VK_QUEUE_GRAPHICS_BIT).queue, 1, &submitInfo, fence);
    // Wait for the fence to signal that command buffer has finished executing
    result = vkWaitForFences(context->GetVkDevice(), 1, &fence, VK_TRUE, UINT32_MAX);
    vkDestroyFence(context->GetVkDevice(), fence, nullptr);
    vkResetCommandBuffer(updateCommandBuffer, VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT);
}
